    ...
    return (
        <>
            <div className={clsx(classes.yesNoWrapper, !isMobile && classes.yesNoWrapperDesktop)}>
                <div className={clsx(classes.questionTitle, !isMobile && classes.questionTitleDesktop)}>{question.labels[language]}</div>
                <div className={clsx(classes.buttonWrapper, !isMobile && classes.buttonWrapperDesktop)}>
                    <BlueButton onClick={() => action( 'ANSWER_SET', {questionId: question.id, data: {answer: 'YES'}})} selected={state === true} size={isMobile ? "small" : ""}>{button_yes[language]}</BlueButton>
                    <div className={classes.buttonSeparator}/>
                    <BlueButton onClick={() => action('ANSWER_SET', {questionId: question.id, data: {answer: 'NO'} })} selected={state === false} size={isMobile ? "small" : ""}>{button_no[language]}</BlueButton>
                </div>
            </div>
            {
                state && question.requiresAdditionalData ?
                    <div className={clsx(classes.additionalDataWrapper, !isMobile && classes.additionalDataWrapperDesktop)}>
                        <div className={clsx(classes.questionTitle, !isMobile && classes.questionTitleDesktop, !isMobile && classes.additionalDataTitleDesktop)}>{question.additionalDataLabels[language]}</div>
                        { additionalDataInput }
                    </div>
                    :
                    ""
